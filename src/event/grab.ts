...
export async function grab(page: play.Page, url: string): Promise<Post | undefined> {
    ...
    try {
        await page.goto(url)
        await page.locator("article > div > div > div > div:nth-child(1) > a > div > div._aagv > img").first().waitFor({ state: "visible", timeout: 15000 })
        await page.locator("span._aacl").first().waitFor({ state: "visible", timeout: 15000 })
        await page.locator("article > div > div > div > div:nth-child(1) > a").first().waitFor({ state: "visible", timeout: 15000 })
        await page.locator("span > img").first().waitFor({ state: "visible", timeout: 15000 })
